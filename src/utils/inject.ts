...
function inject(
  ...
  if (!target || !methodName || !callback) {
    return console.warn(
      `注入方法 -->${target} -- ${methodName} --${callback} 失败`
    );
  }
...
inject(Object.prototype, "merge", function () {
  ...
  if (!isObject(obj) || !isObject(target))
    return console.error("merge方法传入字段必须为object类型", obj, target);
  if (exclude && !isArray(exclude))
    return console.error("merge方法传入字段exclude必须为Array类型");
...
inject(Array.prototype, "flatten", function (this: any[]) {
  function flatten(arr: any[]): any[] {
    return arr.reduce(
      (a, b) =>
        [].concat(
          Array.isArray(a) && a ? flatten(a) : a,
          Array.isArray(b) && b ? flatten(b) : b
        ),
      []
    );
  }
...
inject(
  String.prototype,
  "trim",
  function (this: string, isGlobal: boolean = false): string {
    if (isGlobal) {
      // 是否删除所有空格
