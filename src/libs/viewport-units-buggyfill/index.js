...
function factory() {
  ...
  let viewportUnitExpression = /([+-]?[0-9.]+)(vh|vw|vmin|vmax)/g;
  let urlExpression = /(https?:)?\/\//;
  let forEach = [].forEach;
  let dimensions;
  let declarations;
  let styleNode;
  ...
  let isMobileSafari =
    ...
    (function () {
      ...
      let iOSversion = userAgent.match(/OS (\d+)/); // 这里需要用d+ 而不是d。
  ...
  function findProperties() {
    ...
    forEach.call(document.styleSheets, function (sheet) {
      let cssRules = processStylesheet(sheet);
